...
> AFHS_SIMD<K> {
    ...
    pub fn new() -> AFHS_SIMD<K> {
        AFHS_SIMD::with_capacity(INITIAL_SIZE)
    }

    pub fn with_capacity(size : usize) -> AFHS_SIMD<K> {
        AFHS_SIMD {
    ...
    fn replace_self(&mut self, new_capacity : usize) {
        let mut new_hm : AFHS_SIMD<K> = AFHS_SIMD::with_capacity(new_capacity);
